...
const StrategicOverview = () => {
  const { metrics = {}, zoneStats = [], predictions = {} } = useFleet();
  ...
  const chartData = [
    ...
    { time: '10:00', actual: 3800, predicted: 4500 },
    { time: '12:00', actual: 2900, predicted: 3000 },
    { time: '14:00', actual: 3500, predicted: 3200 },
    { time: '16:00', actual: 4800, predicted: 5000 },
    { time: '18:00', actual: 5200, predicted: 5100 },
  ];
      ...
      <div className="flex justify-between items-end">
        ...
        <div className="flex gap-6 items-center">
           <div className="flex flex-col items-end">
             ...
             <span className="text-lg font-black tracking-tighter text-primary">98.4%</span>
           </div>
           <div className="w-10 h-10 rounded-full border-[3px] border-primary/20 border-t-primary animate-spin"></div>
        </div>
      </div>

      {/* KPI Ribbons - FIXED GRID FOR READABILITY */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { icon: Users, label: 'Day Ridership', value: metrics.passenger_throughput || '42.8k', trend: '+12%', color: 'text-primary', bg: 'bg-primary/10' },
          { icon: Activity, label: 'Fleet Utilization', value: `${metrics.fleet_utilization || 0}%`, trend: 'Optimal', color: 'text-success', bg: 'bg-success/10' },
          { icon: Clock, label: 'Avg Wait Time', value: `${metrics.average_wait_time || 0}m`, trend: '-2.1m', color: 'text-warning', bg: 'bg-warning/10' },
          { icon: Zap, label: 'Efficiency Index', value: metrics.system_efficiency || '0.82', trend: 'Stable', color: 'text-info', bg: 'bg-info/10' }
        ].map((kpi, i) => (
            ...
            {peakZones.map((zone, i) => (
              <div key={i} className="flex flex-col gap-3">
                ...
                <div className="progress-container">
                   <div className="progress-fill bg-primary" style={{ width: `${Math.min(100, (zone.current / zone.predicted) * 100)}%` }}></div>
                </div>
